css_code = (
    ...
    "body::before { content: ''; position: fixed; inset: 0; pointer-events: none; background-image: radial-gradient(var(--outline-variant) 1px, transparent 1px); background-size: 24px 24px; opacity: 0.3; z-index: -1; }\n"
    "body::after { content: ''; position: fixed; inset: 0; pointer-events: none; background: radial-gradient(circle at 70% 30%, rgba(255,255,255,0.8) 0%, transparent 60%), radial-gradient(circle at 30% 80%, rgba(57, 229, 184, 0.05) 0%, transparent 50%); z-index: -1; }\n\n"
    ".app { display: grid; grid-template-columns: 260px minmax(0, 1fr); min-height: 100vh; }\n\n"
    ".side { background: linear-gradient(180deg, var(--primary-container) 0%, #0d1610 100%); color: var(--surface-lowest); display: flex; flex-direction: column; position: relative; z-index: 10; padding: 0 0 24px 0; box-shadow: 4px 0 24px rgba(0,0,0,0.1); }\n"
    ...
    ".blink-dot { width: 6px; height: 6px; background: var(--secondary-fixed); border-radius: 50% !important; box-shadow: 0 0 8px var(--secondary-fixed); animation: pulse 2s infinite; }\n"
    ...
    ".side-edge { position: absolute; right: 0; top: 0; bottom: 0; width: 2px; background: linear-gradient(180deg, transparent, var(--secondary-fixed), transparent); opacity: 0.3; }\n"
    ...
    ".main-top-bar { height: 24px; border-bottom: 1px solid var(--outline-variant); display: flex; align-items: center; padding: 0 48px; gap: 12px; }\n"
    ...
    "/* Dynamic Elements from main.js overrides */\n"
