...
def _translate_tag_to_human_desc(tag: str) -> str:
    ...
    if key == "aoe_damage": return f"群攻：对最多 {parts[3] if len(parts)>3 else (parts[2] if len(parts)>2 else 0)} 人分别造成 {parts[1] if len(parts)>1 else 0}~{parts[2] if len(parts)>2 else 0} 金币伤害"
    ...
    if key == "aoe_cleanse": return f"群辅：为最多 {parts[1] if len(parts)>1 else 0} 人各净化 1 个负面状态"
    if key == "aoe_heal": return f"群辅：为最多 {parts[3] if len(parts)>3 else (parts[2] if len(parts)>2 else 0)} 人分别恢复 {parts[1] if len(parts)>1 else 0}~{parts[2] if len(parts)>2 else 0} 金币"
